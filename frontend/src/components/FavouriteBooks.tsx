...
const FavouriteBooks: React.FC = () => {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        const response = await axios.get<number[]>(`/users/${user.id}/favourite-books`);
        ...
        console.error('Произошла ошибка при попытке загрузки книг.', error);
      }
    };
    const fetchAuthors = async () => {
      try {
        ...
        console.error('Приозошла ошибка при попытке загрузки авторов.', error);
      }
    };
  ...
  const handleRemoveFavouriteBook = async (bookId: number) => {
    try {
      await axios.delete(`/users/${user.id}/favourite-books/${bookId}`);
      setBooks(prevBooks => prevBooks.filter(book => book.id !== bookId));
    } catch (error) {
      console.error('Произошла ошибка при попытке удалить книгу из избранного.', error);
    }
  };
  ...
  return (
    <Tab.Container id="favouritebooks-tabs" defaultActiveKey="favouritebooks">
      <Row>
                ...
                <Card className="mb-2">
                  ...
                  <Card.Body>
                    ...
                    <div className="card-buttons">
                      ...
                      <Button variant="danger" onClick={() => handleRemoveFavouriteBook(book.id)} className="favorite-button">Удалить из избранного</Button>
                    </div>
                  </Card.Body>
                </Card>
              </Col>
